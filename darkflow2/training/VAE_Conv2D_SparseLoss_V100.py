...
from darkflow.networks.VAE_Conv2D import ConvNet
...
model_name = "VAE_Conv2D_SparseLoss_run1_V100_10EP"
num_epochs = 10
num_classes = 1
...
# save the scalers
dump(scaler_p, open(data_save_path + 'darkflow/models/V100/%s_particleScaler.pkl' %model_name, 'wb'))
dump(scaler_met, open(data_save_path + 'darkflow/models/V100/%s_metScaler.pkl' %model_name, 'wb'))
...
for epoch in range(num_epochs):
    ...
    for y, (x_test, wt_test) in tqdm(enumerate(zip(test_loader, weight_test_loader))):
        ...
        #Test
        # print('\rbatch: ',y, end='')
        te_loss, te_kl, te_eucl = test_net(x_test, wt_test)
...
# Save the model
torch.save(model.state_dict(), data_save_path + 'darkflow/models/V100/%s.pt' %model_name)

# store training history
outFile = h5py.File(data_save_path + 'darkflow/models/V100/%s_Traininghistory.h5' %model_name, "w")
