...
class ConvNetRunner:
    def __init__(self, args):
        ...
        if self.flow == 'noflow':
            ...
            self.flow_ID = 'ConvF'
        elif self.flow == 'nsfar':
            self.model = VAE.NSF_AR_VAE(args)
            self.flow_ID = 'NSFAR'
        else:
    ...
    def trainer(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for y, (x_val, met_va, wt_val) in tqdm(enumerate(zip(self.val_loader, self.metVa_loader, self.weight_val_loader))):
                ...
                #Test
                val_loss, val_kl, val_eucl, z_mu, z0, zk = test_convnet(self.model, x_val, met_va, wt_val, batch_size=self.batch_size)
    ...
    def tester(self):
        ...
        for y, (x_test, met_te, wt_test) in tqdm(enumerate(zip(self.test_loader, self.metTe_loader, self.weight_test_loader))):
            ...
            #Test
            te_loss, te_kl, te_eucl, z_mu, z_0, z_k = test_convnet(self.model, x_test, met_te, wt_test, batch_size=self.test_batch_size)
            ...
            self.z0.append(z_0.cpu().detach().numpy())
            self.zk.append(z_k.cpu().detach().numpy())
        ...
        save_npy(np.array(self.z0), self.test_data_save_path + '%s_z0.npy' %self.model_name)
        save_npy(np.array(self.zk), self.test_data_save_path + '%s_zk.npy' %self.model_name)
