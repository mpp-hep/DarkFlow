...
def compute_loss(x, weight, x_decoded, mean, logvar, batch_size=1, beta=1):
    ...
    x_pos = x_pos.view(batch_size, 4, 1, 31) #32 full(13+3), 17-4LJ
    
    x_decoded_pos = torch.zeros(batch_size,4,31)#.cuda()
    # Removes the channel dimension to make the following calculations easier
    ...
    x_decoded_pos = x_decoded_pos.view(batch_size, 4, 31, 1) 
